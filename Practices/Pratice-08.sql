...
Select * from meslekler where soyisim = 'Bulut' order by

-- SORU5: meslekler tablosunda id degeri 5 den büyük olan,
-- ilk 2 veriyi listeleyiniz

Select * from meslekler where id > 5 Fetch next 2 row only 


-- SORU6: meslekler tablosunda maası en yüksek 3 kişinin bilgilerini listeleyiniz

Select * from meslekler Order by maas Desc fetch next 3 row only 
...
Select * from meslekler offset 2 fetch next 5 row only 


-- SORU8: meslekler tablosunda en yüksek maaşı alan 4. 5. 6. kişilerin bilgilerini listele

Select * from meslekler offset 3 fetch next 3 row only 
...
Select a.film_name, a.category, b.actor_name from filmler a Left Join aktorler b on a.film_id = b.id



--1.YOL RİGHT JOİN
 
Select a.film_name, a.category, b.actor_name from filmler a Right Join aktorler b on a.film_id = b.id
...
Select actor_name, film_name from filmler a Left join aktorler b on a.film_id = b.id


--2. YOL RİGHT JOİN

Select actor_name, film_name from filmler a Right join aktorler b on a.film_id = b.id
